...
import subprocess
kinetics = pd.read_csv('kinetics_train.csv')
...
for class_ind, class_name in enumerate(classes_to_download):
    ...
    for video_ind, video_id in enumerate(tqdm(video_ids)):
        ...
        output_filename = 'ytdl' + os.sep + class_name + os.sep + str(video_ind) +'.%(ext)s'
        converted_file = 'ytdl' + os.sep + class_name + os.sep + str(video_ind) + '.avi'
        ...
        try:
            ...
            continue
        source = 'ytdl' + os.sep + class_name + os.sep + str(video_ind) + '.' + info['ext']
        dest = 'ytdl' + os.sep + class_name + os.sep + str(video_ind) + '.avi'
