...
MAX_VIDEOS_PER_CLASS = 10       # limits number of videos to download per class
VIDEO_DURATION = "00:00:10"     # length (seconds) of video downloaded
FRAMES_PER_VIDEO = "5"          # set to download 10 frames for one video in each class
FRAME_RATE = "fps=1/1"          # frames per second(s)

params = {'format': 'bestvideo[height<=480]', 'verbose': 'false'}
ydl = youtube_dl.YoutubeDL(params)  # single instance of ydl class
dt = datetime.now()
Kinetics = pd.read_csv('kinetics_train.csv', nrows=None)
...
def extract_frames(vid_class):
    vid_class = re.sub(r'\s+', '', capwords(vid_class))
    class_files = glob.glob('./train/' + vid_class + '/' '*')
    for video in class_files:
        if is_extracted(video):
            continue
        x, train_or_test, classname, filename = video.split('/')
        filename_no_ext = re.sub(r'\.\w{3}', '', filename)
        src = "%s/%s/%s" % (train_or_test, classname, filename)
        dest = "%s/%s/%s-%%04d.jpg" % (train_or_test, classname, filename_no_ext)
        try:
            ret = subprocess.call(["ffmpeg", "-i", src, "-vframes", FRAMES_PER_VIDEO, dest])
            if ret == 0:
...
def test():
    ...
    num_test_classes = 1      # choose random class
    class_index = list(np.random.choice(len(full_class_list), num_test_classes))
    class_list = [full_class_list[c] for c in class_index]
    for vid_class in class_list:
        ...
        try:
            ...
            download_class_frames(vid_class, class_info)
            move_class_frames(vid_class, vid_list)
...
def main():
    ...
    2. run download_class_frames() to download frames from specified kinetics video class to the python working directory
       defaults: MAX_VIDEOS_PER_CLASS = 1, FRAMES_PER_VIDEO = 5, FRAME_RATE = 1
    3. run move_class_frames to move videos to labelled sub-directories
    4. check 'dl_logging_*.txt' in working directory to view pass/ fail video IDs
    """
    for vid_class in CLASSES_TO_DOWNLOAD:
        ...
        try:
            if MAX_VIDEOS_PER_CLASS < len(class_info):
                vid_index = list(np.random.choice(len(class_info), MAX_VIDEOS_PER_CLASS))
                class_info = [class_info[v] for v in vid_index]
            download_class_videos(vid_class, class_info)
